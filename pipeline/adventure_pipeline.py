"""
Two-phase pipeline for the adventure-story generalization benchmark.

Phase 1 — demo only:
    Gen tasks evaluated with source-split demos as in-context examples.
    No teaching message.

Phase 2 — demo + teaching:
    Same gen tasks, same demos, plus an explicit teaching message.

Variant scaling:
    --num_variants N   : run N independent surface variants (seeds 0..N-1)
    --demo_repeats  K  : show K repetitions of the source pairs in demos
                         (same pairs, different entity names each time)

Usage
-----
    from pipeline.adventure_pipeline import run_two_phase_pipeline
    run_two_phase_pipeline(task_type="c1", elements_path="...", ...)
"""
...
from tree_management.demo_generator import (
    Demo, generate_demos, generate_mixed_demos, build_world_listing,
)
from pipeline.adventure_episode import (
    run_single_episode,
...
def _episode_metrics(result: Dict) -> Dict:
    """Minimal metrics dict for teacher curation from a raw run_single_episode result."""
...
def _format_batch_results(
    ...
    for idx in sorted(raw_results.keys()):
        ...
        formatted = format_episode_result(result_clean, idx)
...
def run_two_phase_pipeline(
    ...
    """
    Run the two-phase generalization pipeline for one task type.

    Parameters
    ----------
    task_type         : "c1" | "c2" | "c3" | "c4"
    elements_path     : path to the base concept elements JSON
    num_variants      : number of independent surface variants to run (default 1)
    demo_repeats      : times to repeat source pairs in demos with different
                        entity names (default 1 = standard single set)
    variants_dir      : where to save generated variant elements files
                        (default: ./variants/<task_type>)
    teaching_strategy : "none" (Phase 2 runs with no teaching message — repeat-run
                        baseline) | static ("react") |
                        dynamic ("hr","idea","ace").
                        Static strategies build per-episode messages from gen task trees.
                        Dynamic strategies call teacher.curate() on Phase 1 failures
                        to produce one shared message via Phase 1.1.
    teacher_model_path: model for ACE teacher calls (defaults to student model_path).

    Returns
    -------
    (phase1_results, phase2_results) — either may be None if skipped/failed
    """
    from tree_management.registry import get_task
    from tree_management.generator import get_template, generate_tasks
    ...
    if num_tries == "max":
        if spec.max_tries is None:
            raise ValueError(
                ...
                "registered. Set max_tries in register_task_type() for this task."
            )
    ...
    from tree_management.elements import fill_elements, load_lexicons, save_variant as _save_variant
    ...
    for variant_seed in range(num_variants):
        ...
        all_demo_source_tasks = list(source_tasks)

        # Shuffle demo tasks reproducibly
        _random.Random(variant_seed).shuffle(all_demo_source_tasks)

        # Coverage sweep: subset demos and gen tasks
        demo_source_tasks = (all_demo_source_tasks[:n_source_demos]
                             if n_source_demos is not None else all_demo_source_tasks)
        ...
        if run_p1:
            ...
            v1 = _phase_summary(eps1, succ1, tok1, variant_seed=variant_seed)
        ...
        if run_p2:
            ...
            v2 = _phase_summary(eps2, succ2, tok2, variant_seed=variant_seed)
...
def run_qa_pipeline(
    ...
    """
    Two-phase Q&A evaluation pipeline over variants.

    Phase 1 — demo only: Q&A with source demos, no teaching message.
    Phase 2 — demo + teaching: same Q&A with a teaching message prepended.

    Shares the same variant/demo-context setup as run_two_phase_pipeline so
    results are directly comparable between Q&A and episode modes.

    Parameters
    ----------
    qa_modes : any subset of ["instance", "structure_mc", "structure_exp"]
        instance      — per gen task: predict item for a specific entity (exact match)
        structure_mc  — per variant: identify the rule type from 5 options (exact match)
        structure_exp — per variant: describe the rule in free form (LLM-as-judge)
    teaching_strategy : "none" | static ("react") |
                        dynamic ("hr","idea","ace").
                        "none": Phase 2 is identical to Phase 1 (skip it unless needed).
    phase1_1_results_file : path to a saved Phase 1.1 JSON (from the episode pipeline)
                            to reuse its teaching message for Q&A Phase 2.
    fixed_teaching_message : if provided, skip all curation and use this string directly
                             as the teaching message for every Phase 2 Q&A question.
                             Takes priority over phase1_1_results_file and teaching_strategy.
    skip_qa_phase1 / skip_qa_phase2 : skip running that phase (must have saved results).

    Returns
    -------
    (qa_phase1_results, qa_phase2_results) — either may be None if skipped/strategy=none.
    Saved to results/qa_phase1_<save_name>.json and results/qa_phase2_<save_name>.json.
    """
    from tree_management.registry import get_task
    from tree_management.generator import get_template, generate_tasks
    from pipeline.qa_episode import (
        run_qa_instance, run_qa_structure_exp,
    )
    from pipeline.prompts import QA_BASE_PROMPT
    ...
    for variant_seed in range(num_variants):
        ...
        # Coverage sweep: apply the same shuffle as the episode pipeline, then subset
        _shuffled_source = list(source_tasks)
        _random.Random(variant_seed).shuffle(_shuffled_source)
        demo_source_tasks = (_shuffled_source[:n_source_demos]
                             if n_source_demos is not None else _shuffled_source)
        eval_gen_tasks    = (gen_tasks[:n_gen_tasks]
        ...
        if run_p1:
            ...
            p1_variant_results.append(
                _run_qa_variant(variant_seed, demo_context, demo_source_tasks, eval_gen_tasks, "",
                                _verbose=verbose)
            )
        ...
        if run_p2:
            ...
            if fixed_teaching_message is None and not variant_teaching_message:
                ...
                if teaching_strategy in STATIC_STRATEGIES:
                    ...
                    if p1_qa:
                        from pipeline.qa_episode import curate_from_qa_results
            ...
            if run_p2:
                ...
                p2_variant_results.append(
                    _run_qa_variant(
                        variant_seed, demo_context, demo_source_tasks, eval_gen_tasks,
                        variant_teaching_message,
                        _verbose=verbose,
                    )
                )
