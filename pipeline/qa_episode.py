"""
Q&A evaluation modes for the generalization benchmark.

Three modes — all use the same demo context as the episode pipeline:

  instance       — one question per gen task: predict the item (property) or extra step
                   (procedural) for a specific entity.
                   Evaluated by exact match on each predicted field.

  structure_mc   — one question per variant: identify the rule type from 5 options.
                   Tests rule-type recognition without revealing the structure.
                   Evaluated by exact choice match.

  structure_exp  — one question per variant: describe the rule in free-form text.
                   Tests whether the model can independently generate the rule.
                   Evaluated by LLM-as-judge against the correct rule description.

Together these decompose understanding (Q&A) from execution (episode success),
and separate recognition (MC) from generation (explanation).

Property tasks:  instance asks for the hidden item name.
Procedural tasks: instance asks for action, argument, ordering, and count of hidden
                  extra step(s).  Structure MC and exp use proc-specific question text.
"""
...
def _get_mc_answer(task_type: str) -> str:
    from tree_management.registry import get_task
    return get_task(task_type).mc_answer
...
def _get_correct_process(task: GeneratedTask) -> Dict[str, Any]:
    """
    Return the hidden extra-step info for a procedural task.

    Traverses tree nodes looking for hidden perform/drink actions, reads their
    execution_order from the out-edge meta to determine ordering relative to buy.

    Returns a dict with keys: action, argument, ordering, count.
    """
    tree = task.tree
    skip_set = set(task.rules_to_skip)
...
def _call_model(
    ...
    """
    One model call. Returns (parsed_dict, raw_response_str).
    Parse order: direct JSON → converter → regex field extraction.
    """
    if verbose:
    ...
    if schema:
        try:
            ...
            return json.loads(converted), raw
        except Exception:
            pass
...
# ---------------------------------------------------------------------------
# Mode 2 — Structure Q&A: multiple choice (disabled)
# ---------------------------------------------------------------------------

# _STRUCTURE_MC_SCHEMA = {
#     "type": "object",
#     "properties": {
#         "choice":    {"type": "string"},
#         "reasoning": {"type": "string"},
...
#     verbose: bool = False,
# ) -> Dict[str, Any]:
#     """Structure Q&A (multiple choice) for one variant."""
#     correct_choice = _get_mc_answer(task_type)
#     question = (QA_STRUCTURE_MC_QUESTION_PROC if _is_proc(task_type)
#                 else QA_STRUCTURE_MC_QUESTION)
#     teaching = format_teaching_block(teaching_message)
#     prompt = "\n\n".join(filter(None, [base_prompt, demo_context])) + teaching
#     prompt = "\n\n".join(filter(None, [prompt, question]))
#
#     parsed, raw  = _call_model(prompt, model, converter_model, _STRUCTURE_MC_SCHEMA,
#                                verbose=verbose, label="structure_mc")
#     predicted    = (parsed.get("choice") or "").strip().upper() if parsed else None
#     reasoning    = (parsed.get("reasoning") or "") if parsed else ""
#     correct      = bool(predicted and predicted == correct_choice)
#
#     return {
#         "mode":            "structure_mc",
#         "task_type":       task_type,
...
# ---------------------------------------------------------------------------
# Mode 3 — Structure Q&A: free-form explanation + LLM judge
# ---------------------------------------------------------------------------
...
def judge_structure_explanation(
    ...
    """
    Score a free-form rule explanation using the model as judge.

    Dispatches to property or procedural judge prompt based on task_type.

    Returns a dict with normalised ``overall`` (0.0–1.0), per-dimension scores,
    ``reasoning``, and ``judge_failed``.

    For property tasks the canonical scores are attribute_score, structure_score,
    generalization_score (max 6 total → overall = sum/6).
    For procedural tasks the canonical scores are input_score, output_score,
    rule_score, generalization_score (max 8 total → overall = sum/8).
    In both cases attribute_score and structure_score are also set on the returned
    dict for pipeline aggregation compatibility:
      property:   attribute_score, structure_score directly from judge
      procedural: attribute_score = input_score, structure_score = rule_score
    """
...
def run_qa_structure_exp(
    ...
    """
    Structure Q&A (free-form explanation) for one variant.

    Uses the proc-specific question (X/Y/g framing) for procedural tasks and
    the proc-specific judge for scoring.

    Returns result dict with explanation, judge scores, and correct rule.
    """
...
def curate_from_qa_results(
    ...
    """
    Curate a teaching message from QA phase 1 results (phase 1.1 for the QA pipeline).

    Uses the QA teacher prompt to ask the model to analyse what the student got wrong
    and write a targeted teaching message — without revealing the answer.

    Parameters
    ----------
    qa_variant_result : result dict for one variant from _run_qa_variant (contains
                        "instance", "structure_mc", and/or "structure_exp" sub-dicts).
    task_type         : task type string used to look up correct_rule.
    model             : model path / identifier for the teacher call.

    Returns
    -------
    Teaching message string (empty string on failure).
    """
