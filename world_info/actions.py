"""
Action registry for the adventure story environment.

To add a new action:
  1. Add an ActionDef entry to ACTION_REGISTRY.
  2. Add a handler method _handle_<name>(self, argument) to AdventureEnv.

ActionDef fields
----------------
name             : str  – action token (e.g. "go", "perform")
generates_rule   : bool – True  → the action appears as a rule *header*
                          ("defeat X requires: ...")
                          False → it appears only as a *dependency* inside
                          another rule, or not at all
dep_display      : str  – f-string template describing how this action's
                          node is shown when it is a *dependency* inside
                          another rule.  Available vars: {arg}, {loc}, {cost}.
                          Empty string → dependency is shown via special-case
                          logic in the rule generator (e.g. "go" → "must be at").
"""
