...
from world_info.actions import ACTION_REGISTRY, get_action, valid_action_names
...
class AdventureEnv:
    ...
    def __init__(
        ...
        """
        Initialize environment with a list of goal trees.

        Args:
            trees: List of (GoalTree, root_id) tuples representing different goals.
                   Pass all relevant trees (gen + source demo trees) so that action
                   validators can find all world items and entities.
            initial_currency: Starting currency
    ...
    def reset(
        ...
        """
        Reset environment and return a formatted initial observation.

        The observation contains:
          - A header line: "=== {task_label}: {action} {entity} ==="
          - One RPG card per non-hidden tree node that has displayable properties
            (attribute1/2 name+value, location, cost).

        Pass task_label="Episode" when generating demo context.
    ...
    def step(self, action: str, argument: str) -> Tuple[Optional[str], Observation, Dict[str, float]]:
        """
        Execute action and return (full_action, observation, completion_map).
        
        Args:
            action: Action name (e.g., "go", "get", "buy", "defeat", "rescue")
            argument: Action argument (e.g., location name, item name, enemy name)
            
        Returns:
            Tuple of (full_action_string, Observation, completion_map)
            where completion_map is {node_id: completion_ratio (0.0 to 1.0)}
        """
        if self.done:
    ...
    def _apply_search_node_replacement(self, item: str) -> Optional["Observation"]:
        ...
        if not existing_item:
            return None
        self.inventory[existing_item] = 0
